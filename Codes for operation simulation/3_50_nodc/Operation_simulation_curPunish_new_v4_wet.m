...
% for iy = 1 : ny
%     coeffXu=coeff.*(repmat(X(:,iy)',1,nt*2*2));
%     coeffX([1:ns*nj]+(iy-1)*ns*nj,1:nlc*nt*2*2)=repmat(coeffXu,ncalu/ny,1);
% end
...
for imonth= 1 : n_month
    ...
    for ih= 1 : n_hydro
        ...
        B_hydro(n_row_h,1)=mpc.hydro.wet(ih,imonth)*24*sum(dj(idx_j_h));
    end
